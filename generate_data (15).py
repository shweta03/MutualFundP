...
BENCHMARK_TICKERS = {
    ...
    "smallcap100":("^NSESC100",  "Nifty Smallcap 100","2003-01-01"),
}
...
def run_backtest(profile_key, eq_ratio, debt_ratio, sg_history,
    ...
    # Realistic historical fallback returns (Nifty 50 approximate)
    # Used when fund has no actual annual_rets for a given year
    EQ_FALLBACK = {
        2013: 6.8,  2014: 31.4, 2015: -4.1, 2016: 3.0,
        2017: 28.6, 2018: 3.2,  2019: 12.0, 2020: 14.9,
        2021: 24.1, 2022: 4.3,  2023: 20.0, 2024: 8.8,
        2025: 10.1, 2026: -6.9,
    }
    DEBT_FALLBACK = {
        2013: 3.5, 2014: 14.1, 2015: 7.6, 2016: 14.5,
        2017: 4.5, 2018: 6.1,  2019: 11.2, 2020: 11.8,
        2021: 3.4, 2022: 2.8,  2023: 7.2,  2024: 8.1,
        2025: 7.5, 2026: 7.8,
    }
    GOLD_FALLBACK = {
        2013: -12.6, 2014: -1.8, 2015: -9.7, 2016: 11.5,
        2017: 5.3,   2018: 7.8,  2019: 22.6,  2020: 28.0,
        2021: -3.6,  2022: 11.2, 2023: 15.1,  2024: 21.3,
        2025: 18.0,  2026: 14.2,
    }
    ...
    for year in range(START_YEAR, end_year + 1):
        ...
        # Fallbacks use realistic historical returns — NOT flat positive values
        # Flat fallbacks (12%, 7.5%) cause MaxDD = 0 since nav never drops
        if eq_ret  is None: eq_ret  = EQ_FALLBACK.get(year, 10.0)
        if dt_ret  is None: dt_ret  = DEBT_FALLBACK.get(year, 7.0)
        if gld_ret is None: gld_ret = GOLD_FALLBACK.get(year, 8.0)
