...
def run_backtest(profile_key, eq_ratio, debt_ratio, sg_history,
    ...
    """
    Deterministic annual backtest using ONLY actual fund annual returns.
    Option B: starts from the earliest year ALL selected funds have real data.
    No fallbacks — if no data for a year, that year is skipped.
    This ensures MaxDD, CAGR, Sharpe are 100% accurate — no synthetic numbers.
    """

    def get_avg_annual(funds, year):
        """Average annual return across funds for a given year. None if no data."""
        rets = [f.get("_annual_rets", {}).get(year)
                for f in funds
                if f.get("_annual_rets", {}).get(year) is not None]
        return round(sum(rets) / len(rets), 2) if rets else None

    def earliest_data_year(funds):
        """Find the earliest year where at least one fund has data."""
        years = []
        for f in funds:
            ar = f.get("_annual_rets", {})
            if ar:
                years.append(min(int(y) for y in ar.keys()))
        return min(years) if years else date.today().year

    # Start backtest from the year we have real data for
    all_funds = eq_funds + dt_funds + gold_funds
    start_year = earliest_data_year(all_funds)
    ...
    for year in range(start_year, end_year + 1):
        ...
        # Skip year entirely if no equity data available — don't use fallbacks
        if eq_ret is None and eq_ratio > 0:
            continue
        if dt_ret is None and debt_ratio > 0:
            continue

        # Use 0 for gold if missing (gold is a small allocation)
        if gld_ret is None:
            gld_ret = 0.0

        # SG ratio at start of year (Dec-31 of previous year)
        prev_year_ratio = sg_history.get(str(year - 1),
                          sg_history.get(str(year), 9.0))
        gold_active = float(prev_year_ratio) > GOLD_THRESHOLD
        gf = GOLD_ACTIVE_SPLIT if gold_active else (1 - GOLD_ACTIVE_SPLIT)
        ef = 1 - gf

        blended_eq = (eq_ret or 0) * ef + gld_ret * gf
        port_ret   = eq_ratio * blended_eq + debt_ratio * (dt_ret or 0)
    ...
    return {
        ...
        "win_rate":   win_rate,
        "n_years":    n,
        "start_year": rows[0]["year"] if rows else start_year,
...
def main():
    ...
    # Write data.json — build string first to avoid partial writes
    import io as _io
    _buf = _io.StringIO()
    json.dump(data_out, _buf, indent=2, allow_nan=True)
    _content = _buf.getvalue()
    # Replace any NaN/Infinity with null (not valid JSON for browsers)
    _content = (_content.replace(': NaN', ': null')
                        .replace(':NaN', ':null')
