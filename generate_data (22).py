...
def select_portfolio_funds(scored_funds, profile_key, nifty_1y, nifty_3y, nifty_5y):
    ...
    if profile_key == "A":
        ...
        # Slot 3 — Large Cap / Flexi Cap / International momentum (different AMC from s1, s2)
        s3 = top("Equity", ["Large Cap", "Flexi Cap", "International"], 1,
                 exclude_ids=ids(picks), exclude_amcs=amcs(s1 + s2),
                 require_momentum=True)
        if not s3:  # fallback: try international funds
            s3 = top("International", ["fof overseas", "international", "global"], 1,
                     exclude_ids=ids(picks), exclude_amcs=amcs(s1 + s2),
                     require_momentum=True)
        picks += s3
...
def run_backtest(profile_key, eq_ratio, debt_ratio, sg_history,
    ...
    # Mid/Small cap fallback (more volatile — used when equity fund is mid/small)
    MIDSMALL_FALLBACK = {
        2013: -2.8, 2014: 46.1, 2015: 6.3,  2016: 3.4,
        2017: 47.1, 2018:-10.3, 2019: -4.5, 2020: 26.7,
        2021: 43.2, 2022: 4.1,  2023: 39.5, 2024: 18.2,
        2025: 3.1,  2026:-12.4,
    }
    # Debt: banking+PSU/short duration blend
    DEBT_FALLBACK = {
        2013: 5.2,  2014: 12.8, 2015: 7.6,  2016: 10.8,
        2017: 5.4,  2018: 5.9,  2019: 10.2, 2020: 11.4,
        2021: 4.2,  2022: 2.9,  2023: 7.5,  2024: 8.1,
        2025: 7.8,  2026: 8.1,
    }
    # Gold
    GOLD_FALLBACK = {
