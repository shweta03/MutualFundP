...
def compute_raw_score(m, asset_type):
    ...
    # sf=True  → Sharpe >= 1.0 (good)
    # df=True  → Max DD better than -25% (good; Gold exempt)
    # fp=True  → passes BOTH filters (qualifying fund)
...
def select_portfolio_funds(scored_funds, profile_key, nifty_1y, nifty_3y, nifty_5y):
    ...
    def top(asset_type, keywords, n=1, exclude_ids=None, exclude_amcs=None,
        ...
        # Max DD cap: -50% for Aggressive, -40% for Moderate/Conservative
        # Prevents extreme-DD funds (e.g. -83% JM Value) from entering portfolios
        ...
        # If momentum/mean_rev filter yields nothing, fall back without it (keep DD cap)
