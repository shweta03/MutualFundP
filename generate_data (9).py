...
# AMFI category: "Other Scheme - FoF Overseas"
...
def fetch_aum():
    """
    Fetch scheme-wise AUM from AMFI monthly data file.
    AMFI publishes this on the 7th of each month for previous month.
    URL pattern: https://www.amfiindia.com/spages/NAVAll.txt companion
    
    We use the AMFI Average AUM (AAUM) data which is the standard
    used by all platforms. Returns dict: scheme_code → aum_crores (float)
    
    Falls back gracefully to empty dict if unavailable.
    """
    print("📊 Fetching AUM data from AMFI...")

    # AMFI publishes monthly AAUM in a fixed-format text file
    # URL: https://www.amfiindia.com/modules/AumLoad?mf=0&tp=1&frm=N
    # Format is pipe-separated: SchemeCode|SchemeName|AAUM|AUM
    
    # Try multiple months in case current month isn't published yet
    ...
    for months_back in range(0, 3):
        ...
        month_name = target.strftime("%b")  # "Apr", "Mar" etc
        
        # AMFI scheme-wise AAUM URL formats
        urls_to_try = [
            f"https://www.amfiindia.com/modules/AumLoad?mf=0&tp=1&frm=N",
            f"https://portal.amfiindia.com/DownloadData_Po.aspx?mf=0&tp=1&Month={month_num}&Year={target.year}",
        ]
        ...
        for url in urls_to_try:
            try:
                ...
                # Parse — format: Code;Name;AAUM;AUM or pipe-separated
                separator = ";" if ";" in r.text[:500] else "|"
                found = 0
                for line in r.text.splitlines():
                    ...
                    if len(parts) >= 3:
                        try:
                            code = int(parts[0].strip())
                            # AUM is usually the 3rd or 4th column in crores
                            aum_val = float(parts[2].strip().replace(",", ""))
                            if aum_val > 0:
                ...
                if found > 100:
                    print(f"   ✅ AUM loaded: {found} schemes ({month_name} {target.year})")
                    return aum_map
                    
            except Exception as e:
                continue
    
    # If all AMFI endpoints fail, try mfapi scheme search for AUM
    # mfapi doesn't have AUM but we can estimate from NAV × units
    # For now return empty — screener will show N/A for AUM
    print("   ⚠ AUM fetch failed — will show N/A for AUM (AMFI endpoint unavailable)")
    return {}
...
def main():
    ...
    print(f"   Valid candidates: {len(candidates)} "
          ...
          f"{type_counts['International']} International)")
    ...
    for idx, fund in enumerate(candidates, 1):
        ...
        funds_raw.append({
            ...
            "nav_date":    nav_date,
            "aum":         aum_map.get(code),   # crores, None if unavailable
            "live":        True,
