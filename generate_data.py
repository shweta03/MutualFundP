...
def main():
    ...
    for idx, fund in enumerate(candidates, 1):
        ...
        hist = fetch_history(code, nav_date, min_years=0)
        time.sleep(0.2)  # respectful rate limiting

        if hist is None:
            # mfapi has no history at all → exclude
            skipped_history += 1
            fund_id_counter += 1
            continue
        ...
        funds_raw.append({
            ...
            "new_fund":    years_available < 3,   # flag for screener badge
            "r1m":         metrics["r1m"],
            "r3m":         metrics["r3m"],
            "r1":          metrics["r1"],
            "r3":          metrics["r3"],
    ...
    print(f"   Skipped: {skipped_history} (no mfapi history)")
    ...
    data_out = sanitize({
        ...
        "funds":           funds_clean,
        "amfi_all":        amfi_all,   # ← full AMFI universe for Review PF search
    })

    with open("data.json", "w") as f:
        json.dump(data_out, f, indent=2)
    print(f"\n✅ data.json written ({live_count} live funds, {len(funds_clean)} total, {len(amfi_all)} in amfi_all)")
