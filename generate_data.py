...
def run_backtest(profile_key, eq_ratio, debt_ratio, sg_history,
    ...
    START_YEAR = 2000   # Extended to 2000 for full history; fallbacks cover pre-fund years

    # ── Category-specific fallbacks (actual index returns) ──────────────
    # Used when a fund has no data for a year (launched after that year)
    # Equity: Nifty 50 actual annual returns
    EQ_FALLBACK = {
    ...
    # Gold: actual INR gold price annual returns
    GOLD_FALLBACK = {
    ...
    if len(all_months) >= 12:
        ...
        # For months where a fund has no data (e.g. Gold ETF before 2023),
        # use the annual fallback divided by 12 as a monthly approximation
        gold_ratio = 1.0 - eq_ratio - debt_ratio
        ...
        for month in all_months:
            ...
            # Fallback to annual/12 when fund has no monthly data for this period
