...
def main():
    ...
    # Shared session for connection reuse (faster than new connection per fund)
    mfapi_session = requests.Session()
    mfapi_session.headers.update({"User-Agent": "Mozilla/5.0 (compatible; MintingM/1.0)"})

    # Circuit breaker: if mfapi keeps timing out, pause and let it recover
    ...
    for idx, fund in enumerate(candidates, 1):
        ...
        if hist is None:
            ...
            if consecutive_timeouts >= MAX_CONSECUTIVE_TIMEOUTS:
                print(f"   ⚠ {consecutive_timeouts} consecutive failures — pausing 30s for mfapi recovery...")
                time.sleep(30)
                consecutive_timeouts = 0
            skipped_history += 1
            fund_id_counter += 1
