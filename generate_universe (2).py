...
EQUITY_SUBCATS = [
    "flexi cap","large cap","mid cap","small cap","large & mid cap","multi cap",
    "focused fund","contra fund","value fund","elss","balanced advantage",
    "dynamic asset allocation","aggressive hybrid","equity savings","conservative hybrid",
    "sectoral fund","thematic fund","banking","financial services","infrastructure",
    "technology","pharma","healthcare","consumption","energy","manufacturing",
    "psu","mnc","dividend yield","esg","global","overseas","index fund","index",
]
DEBT_SUBCATS = [
    "short duration","corporate bond","banking and psu","medium duration",
    "low duration","money market","ultra short duration",
    "liquid","overnight","long duration","gilt","floater","credit risk",
    "dynamic bond","medium to long",
]
GOLD_SUBCATS   = ["gold etf","gold fund","gold fof","gold savings"]
HYBRID_SUBCATS = ["multi asset allocation","arbitrage","balanced hybrid"]
...
def get_asset_type(amfi_cat):
    ...
    if any(k in sub for k in HYBRID_SUBCATS): return "Hybrid"
...
def main():
    ...
    import io as _io
    _buf = _io.StringIO()
    json.dump(data, _buf, indent=2, allow_nan=True)
